...
func main() {
	...
	db.Exec(`CREATE TABLE IF NOT EXISTS available_bonds (id SERIAL PRIMARY KEY, name TEXT, price FLOAT, rate FLOAT)`)
	...
	b.Handle(telebot.OnWebApp, func(c telebot.Context) error {
		...
		switch d.Action {
		case "register":
			_, err := db.Exec("INSERT INTO users (tg_id, nickname, role) VALUES ($1, $2, $3) ON CONFLICT (tg_id) DO UPDATE SET nickname=$2, role=$3", uid, d.Nick, d.Role)
